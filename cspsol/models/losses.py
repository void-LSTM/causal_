...
class CMIEstimator(nn.Module):
    ...
    def forward(self, z_t: torch.Tensor, z_m: torch.Tensor, y: torch.Tensor, 
        ...
        if z_m_for_head.shape[1] < z_tzm.shape[1]:
            padding = torch.zeros(batch_size, z_tzm.shape[1] - z_m_for_head.shape[1], 
                                device=z_m_for_head.device, dtype=z_m_for_head.dtype)
            z_m_padded = torch.cat([z_m_for_head, padding], dim=1)
        else:
            z_m_padded = z_m_for_head
        
        # Forward through trunk
        h_tzm = self.trunk(z_tzm)
        h_zm = self.trunk(z_m_padded)
...
class LossMAC(nn.Module):
    ...
    def forward(self, z: torch.Tensor, a: torch.Tensor) -> torch.Tensor:
        ...
        if num_pairs == max_possible_pairs:
            ...
            # Sample random pairs
            i_indices = torch.randint(0, batch_size, (num_pairs,), device=z.device)
            j_indices = torch.randint(0, batch_size, (num_pairs,), device=z.device)
            # Ensure i != j
